...
async fn dispatch_event(
    ...
    let fat = utils::write_scratch(store, exports.memory, &exports, scratch, payload)
        .await?;

    let result_ptr = FatPtr::unpack(
        handler
            .call_async(&mut *store, fat.pack())
            .await?
    );

    utils::dealloc_scratch(store, &exports.instance, fat)
        .await?;
